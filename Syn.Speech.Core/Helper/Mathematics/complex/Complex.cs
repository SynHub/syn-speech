 ...
 *
 */
public class Complex : FieldElement<Complex>, ISerializable  {
    /** The square root of -1. A number representing "0.0 + 1.0i" */
    public readonly Complex I = new Complex(0.0, 1.0);
    // CHECKSTYLE: stop ConstantName
    /** A complex number representing "NaN + NaNi" */
    public static readonly Complex NaN = new Complex(Double.NaN, Double.NaN);
    // CHECKSTYLE: resume ConstantName
    /** A complex number representing "+INF + INFi" */
    public readonly Complex INF = new Complex(Double.PositiveInfinity, Double.PositiveInfinity);
    ...
    public Complex(double real, double imaginary) {
        ...
        _isNaN = Double.IsNaN(real) || Double.IsNaN(imaginary);
        _isInfinite = !_isNaN &&
    ...
    public Complex divide(double divisor) {
        if (_isNaN || Double.IsNaN(divisor)) {
            return NaN;
        }
        if (divisor == 0d) {
            return NaN;
    ...
    public Complex reciprocal() {
        ...
        if (Math.Abs(real) < Math.Abs(imaginary)) {
            double q = real / imaginary;
            double scale = 1f / (real * q + imaginary);
            return createComplex(scale * q, -scale);
        } else {
            double q = imaginary / real;
            double scale = 1f / (imaginary * q + real);
     ...
     * @since 3.3
     */
    public static bool equalsWithRelativeTolerance(Complex x, Complex y,
                                                      double eps) {
    ...
    public Complex subtract(double subtrahend) {
        if (_isNaN || Double.IsNaN(subtrahend)) {
            return NaN;
        }
    ...
    public Complex sqrt() {
        ...
        if (real >= 0.0) {
            ...
            return createComplex(Math.Abs(imaginary) / (2.0 * t), FastMath.copySign(1d, imaginary) * t);
        }
    }
     ...
     */
    public Complex tan() {
        if (_isNaN || Double.IsInfinity(real)) {
            return NaN;
        }
        if (imaginary > 20.0) {
     ...
     */
    public Complex tanh() {
        if (_isNaN || Double.IsInfinity(imaginary)) {
            return NaN;
        }
        if (real > 20.0) {
